...
class IslandMaker(object):
    ...
    def layout_first_two_points(self,meshLoc,start_frame):
        ...
        if self.myMesh.is_naked_edge(meshEdge):
            at_z_zero = self.myMesh.edge_is_in_xy_plane(edge=meshEdge, thresh=.0001)
            self.island.change_to_naked_edge(islandEdge,in_xy_plane=at_z_zero)
        ...
        while True:
            ...
            for i,orientedEdge in enumerate(orientedEdges):
                ...
                if self.myMesh.is_fold_edge(edge):
                    ...
                    at_z_zero = self.myMesh.edge_is_in_xy_plane(edge, thresh=.0001)
